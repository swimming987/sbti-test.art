...
const typePatches = {
  CTRL: {
    ...
    desc: 'You are one of the rarest personality types out there. You are a natural-born rebel against chaos. For you, “rules” are just default settings and plans are more like rough sketches. A CTRL friend is the person who keeps your life on track: precise, efficient, and impossible to ignore. When things are about to go off the rails, CTRL is the one who hits Ctrl+S, pulls everyone back, and saves the timeline.'
  },
  'ATM-er': {
    cn: 'The Giver',
    intro: 'You think I’m rich?',
    desc: 'You are one of the rarest personalities in the world. If there is a financial mystery here, it is that you are not necessarily “giving money” so much as constantly paying with time, energy, patience, and peace of mind. Like an old but sturdy ATM, you absorb other people’s anxiety and return one thing in exchange: “Don’t worry, I’ve got it.” Your life is one long, unglamorous solo-payment performance.'
  },
  'Dior-s': {
    cn: 'The Slacker',
    intro: 'Just wait for my underdog comeback.',
    desc: 'Congratulations, you are not a loser in the ordinary sense. You are more like the spiritual heir of Diogenes: a hard-edged realist who sees straight through consumerism and success-culture nonsense. Dior-s people are not short on ambition; they simply know that the end of “getting ahead” is often just a nicer cage. Their wisdom is simple: lying down is more comfortable than standing, and when it is time to eat, you eat.'
  },
  BOSS: {
    cn: 'The Leader',
  ...
  'THAN-K': {
    ...
    desc: 'You are one of the rarest personalities in China. THAN-K people can thank the heavens for almost anything. Even if traffic jams your commute, you can still thank the delay for the extra time, the music, and the chance to watch the anxious faces outside the window. They are warm, generous, and strangely hard to hate. In their world, there are no fully bad people - only friends who have not yet been touched by gratitude.'
  },
  'OH-NO': {
    cn: 'The Alarmist',
  ...
  GOGO: {
    ...
    desc: 'GOGO people live in a world of pure cause and effect. Their creed is brutally simple: if I close my eyes, it is dark; if I spend all my money, I have no money left; if I stand on the crosswalk, I am a pedestrian. Their logic is perfectly closed-loop, impossible to argue with. While others debate “chicken or egg,” GOGO has already turned both into dinner and moved on to the next task.'
  },
  SEXY: {
    cn: 'The Charmer',
    intro: 'You were born to be irresistible!',
    desc: 'When you walk into a room, the lighting system would politely dim itself to avoid wasting energy on your natural charisma. When you smile, the air seems to get warmer, as if moisture itself had turned into heart shapes. SEXY people do not need to force their presence; simply existing already feels like an overly glamorous poem. If enough of them gather for a party, the atmosphere might bend time just a little.'
  },
  'LOVE-R': {
    cn: 'The Lover',
    intro: 'Your love is overflowing; reality feels underfed.',
    desc: 'LOVE-R is like a rare species that somehow survived from ancient myth into the present day. Your emotional processor is not binary; it runs on rainbow logic. A falling leaf is never just “autumn” to you - it is a whole tragedy about rebirth, sacrifice, and wordless love. Your inner world is an amusement park that never closes, and you spend your life looking for someone who can read the map and ride the carousel to the end of the universe with you.'
  },
  MUM: {
    cn: 'Mom',
    intro: 'Maybe... can I call you mom...?',
    desc: 'Congratulations, you are one of the rarest “mom” personalities. Before time had a name, before the first star even existed, Mom was already there. This type is gentle, emotionally aware, and deeply empathetic. Mom knows when to stop, when to pause, and when to say, “Fine, let it go.” MUM people often use their warmth to heal others, but the medicine they give themselves is always a smaller dose.'
  },
  FAKE: {
    cn: 'The Pretender',
  ...
  DEAD: {
    ...
    desc: 'Congratulations, you got one of the rarest personalities in China. The name sounds unlucky, so you can also think of it as “Don’t Expect Any Drives.” DEAD has seen through all the pointless philosophy and has lost interest in anything that looks like gain or loss. They look at the world like a top player who has beaten every main quest, side quest, and hidden mission so many times that they finally realize the game itself is the joke. DEAD is the ultimate sage beyond desire and goals.'
  },
  IMFW: {
...
const types = readJSON('types-i18n.json');
for (const [code, patch] of Object.entries(typePatches)) {
  if (!types.types[code]) continue;
  if (patch.cn) types.types[code].cn.en = patch.cn;
  if (patch.intro) types.types[code].intro.en = patch.intro;
  if (patch.desc) types.types[code].desc.en = patch.desc;
}
writeJSON('types-i18n.json', types);

const dimensions = readJSON('dimensions-i18n.json');
for (const [dim, patch] of Object.entries(dimPatches.dimensionMeta)) {
  if (!dimensions.dimensionMeta[dim]) continue;
  dimensions.dimensionMeta[dim].name.en = patch.name;
  dimensions.dimensionMeta[dim].model.en = patch.model;
}
for (const [dim, levels] of Object.entries(dimPatches.dimExplanations)) {
  if (!dimensions.dimExplanations[dim]) continue;
  for (const [level, text] of Object.entries(levels)) {
    if (dimensions.dimExplanations[dim][level]) {
      dimensions.dimExplanations[dim][level].en = text;
    }
  }
}
writeJSON('dimensions-i18n.json', dimensions);

console.log('Polished English translations updated.');
